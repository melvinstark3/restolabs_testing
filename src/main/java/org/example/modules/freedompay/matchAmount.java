...
public class matchAmount extends org.example.commonUtils.payment.matchAmount {
    public void matchAmount(String checkoutOrderTotal) {
        ...
        boolean laravelPage = false;
        try {
            laravelPage = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h4[@class=\"total__amount\"]"))).isDisplayed();
            System.out.println("Laravel Page is Displayed");
        } catch (TimeoutException e){
            System.out.println("Gateway Page is Displayed");
        }
        if (laravelPage){
